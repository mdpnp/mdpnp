...
public class SimulatedPulseOximeterImpl extends AbstractSimulatedConnectedDevice implements SimulatedPulseOximeter {
	
	private final MutableNumericUpdate pulseUpdate = new MutableNumericUpdateImpl(PULSE);
	private final MutableNumericUpdate spo2Update = new MutableNumericUpdateImpl(SPO2);
	private final MutableWaveformUpdate plethUpdate = new MutableWaveformUpdateImpl(PLETH);
...
//	private MyTask myTask;
	...
	private final class MyTask implements Runnable {
		...
		@Override
		public void run() {
//			count=(count=count+25)>=pleth.length?0:count;
			Number[] values = plethUpdate.getValues();
			for(int i = 0; i < values.length; i++) {
				values[i] = pleth[postIncrCount()];
			}
...
//				offset = offsetRandom.nextInt(pleth.length);
			...
			pulseUpdate.set(currentDraw.getHeartRate(), myDate);
			spo2Update.set(currentDraw.getSpO2(), myDate);
			gateway.update(spo2Update, pulseUpdate, plethUpdate);
		}
	...
	private void initPleth() {
		...
		plethUpdate.setMillisecondsPerSample(13.333333333);
		plethUpdate.setValues(new Number[25]);
	}
	
//	private final java.util.Timer timer = new Timer(true);
//	private final ScheduledExecutorService executor = Executors.newSingleThreadScheduledExecutor();
	...
	public void connect(String str) {
		...
		task = executor.scheduleAtFixedRate(new MyTask(), 10L, 334L, TimeUnit.MILLISECONDS);
	...
	public SimulatedPulseOximeterImpl(Gateway gateway) {
		super(gateway);
