 ...
 * @since JavaFX 2.1
 */
public class FixedComboBox<T> extends ComboBoxBase<T> {
     ...
     */
    public FixedComboBox(ObservableList<T> items) {
    ...
    private ObjectProperty<StringConverter<T>> converter = 
            new SimpleObjectProperty<StringConverter<T>>(this, "converter", FixedComboBox.<T>defaultStringConverter());
    ...
    static class ComboBoxSelectionModel<T> extends SingleSelectionModel<T> {
        private final FixedComboBox<T> comboBox;

        public ComboBoxSelectionModel(final FixedComboBox<T> cb) {
            if (cb == null) {
