...
public class DiagnosticApplicationFactory implements IceApplicationProvider {
	private final IceApplicationProvider.AppType DiagnosticApplication = new IceApplicationProvider.AppType(
			"System Explorer", "NODIAG", (URL) DiagnosticApplicationFactory.class.getResource("diag.png"), 0.75, false);
	...
	public IceApplicationProvider.IceApp create(ApplicationContext parentContext) throws IOException {
		...
		final LocalAlarmLimitObjectiveFxList localAlarmLimitObjectiveList = parentContext
				.getBean("localAlarmLimitObjectiveList", LocalAlarmLimitObjectiveFxList.class);

		final GlobalAlarmLimitObjectiveFxList globalAlarmLimitObjectiveList = parentContext
				.getBean("globalAlarmLimitObjectiveList", GlobalAlarmLimitObjectiveFxList.class);

		final ValidationOracle validationOracle = parentContext.getBean("validationOracle", ValidationOracle.class);

		final InfusionStatusFxList infusionStatusList = parentContext.getBean("infusionStatusList",
				InfusionStatusFxList.class);

		final BypassStatusFxList bypassStatusList = parentContext.getBean("bypassStatusList", BypassStatusFxList.class);

		final Diagnostic diagnostic = new Diagnostic(patientAlertList, technicalAlertList, numericList, sampleArrayList,
				validationOracle, infusionStatusList, bypassStatusList, alarmLimitList, localAlarmLimitObjectiveList,
