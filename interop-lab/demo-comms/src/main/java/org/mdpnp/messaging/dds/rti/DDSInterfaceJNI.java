...
public class DDSInterfaceJNI {
	...
	public DDSInterfaceJNI() {
		if(0==RTICLibrary.INSTANCE.DDS_OctetSeq_initialize(readOctetSequence.getPointer())) {
			throw new RuntimeException("failed to initialize DDS_OctetSeq");
		}
		readOctetSequence.read();
		if(0==RTICLibrary.INSTANCE.DDS_OctetSeq_initialize(writeOctetSequence.getPointer())) {
			throw new RuntimeException("failed to initialize DDS_OctetSeq");
		}
		writeOctetSequence.read();
		if(0==RTICLibrary.INSTANCE.DDS_DoubleSeq_initialize(readDoubleSequence.getPointer())) {
			throw new RuntimeException("failed to initialize DDS_DoubleSeq");
		}
		readDoubleSequence.read();
		if(0==RTICLibrary.INSTANCE.DDS_DoubleSeq_initialize(writeDoubleSequence.getPointer())) {
			throw new RuntimeException("failed to initialize DDS_DoubleSeq");
		}
	...
	public final void arrayToSequence(byte[] bytes, RTICLibrary.DDS_Sequence seq) {
		...
		if(0 == RTICLibrary.INSTANCE.DDS_OctetSeq_ensure_length(seq.getPointer(), bytes.length, bytes.length)) {
			throw new RuntimeException("Unable to ensure_length");
		}
		
		for(int i = 0; i < bytes.length; i++) {
			Pointer p = RTICLibrary.INSTANCE.DDS_OctetSeq_get_reference(seq.getPointer(), i);
			if(null == p) {
	...
	public final void arrayToSequence(Number[] numbers, RTICLibrary.DDS_Sequence seq) {
//		if(0 == RTICLibrary.INSTANCE.DDS_DoubleSeq_initialize(seq.getPointer())) {
//			throw new RuntimeException("Unable to initialize sequence");
//		}
		if(0 == RTICLibrary.INSTANCE.DDS_DoubleSeq_ensure_length(seq.getPointer(), numbers.length, numbers.length)) {
			throw new RuntimeException("Unable to ensure_length");
		}
		for(int i = 0; i < numbers.length; i++) {
			Pointer p = RTICLibrary.INSTANCE.DDS_DoubleSeq_get_reference(seq.getPointer(), i);
			if(null == p) {
				throw new RuntimeException("Unable to get_reference");
			}
			if(null == numbers[i]) {
	...
	public final void arrayToSequence(String[] strings, RTICLibrary.DDS_Sequence seq) {
		if(0 == RTICLibrary.INSTANCE.DDS_StringSeq_initialize(seq.getPointer())) {
			throw new RuntimeException("Unable to initialize sequence");
		}
		if(0 == RTICLibrary.INSTANCE.DDS_StringSeq_ensure_length(seq.getPointer(), strings.length, strings.length)) {
			throw new RuntimeException("Unable to ensure_length");
		}
		for(int i = 0; i < strings.length; i++) {
			Pointer p = RTICLibrary.INSTANCE.DDS_StringSeq_get_reference(seq.getPointer(), i);
			if(null == p) {
				throw new RuntimeException("Unable to get_reference");
			}
	...
	public void get(Pointer dd,  MutableIdentifiableUpdate<?> update) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		for(DDSInterfaceJNI.Getter m : populateGetters(update.getClass()).getGetters()) {
			
			Method setter = m.getSetter();
			if(null == setter) {
				setter = update.getClass().getMethod("s"+m.getName().substring(1, m.getName().length()), m.getReturnType());
				m.setSetter(setter);
			}
			setter.invoke(update,  get(m.getName(), dd, setter.getParameterTypes()[0], m.getMemberId(), m.getKind()));
		}
	}
	private static final Logger log = LoggerFactory.getLogger(DDSInterfaceJNI.class);
	...
	public final Object get(String name, Pointer dd, Class<?> tgt, int member_id, int kind) {
		if(RTICLibrary.DDS_TK_STRING==kind) {
			...
			if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_string(dd, pbr, strlen, null, member_id))) {
				return null;
			}
			...
			if(Enum.class.isAssignableFrom(tgt)) {
				...
				try {
					...
					Enum<?> en = (Enum<?>) enumClass.getMethod("valueOf", String.class).invoke(null, s.substring(idx+1, s.length()));
					return en;
				} catch (Exception e) {
					e.printStackTrace();
					return null;
			...
			if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_longlong(dd, longByReference, null, member_id))) {
				return null;
			}
			...
			if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_long(dd, intByReference, null, member_id))) {
				return null;
			} else {
				return intByReference.getValue();
			}
		} else if(RTICLibrary.DDS_TK_SHORT==kind) {
			if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_short(dd, shortByReference, null, member_id))) {
				return null;
			} else {
				return shortByReference.getValue();
			}
		} else if(RTICLibrary.DDS_TK_CHAR==kind) {
			if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_octet(dd, byteByReference, null, member_id))) {
				return null;
			} else {
				return byteByReference.getValue();
			}
		} else if(RTICLibrary.DDS_TK_DOUBLE==kind) {
			if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_double(dd, doubleByReference, null, member_id))) {
				return null;
			} else {
				return doubleByReference.getValue();
			}
		} else if(RTICLibrary.DDS_TK_FLOAT==kind) {
			if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_float(dd, floatByReference, null, member_id))) {
				return null;
			} else {
				return floatByReference.getValue();
			}
		} else if(RTICLibrary.DDS_TK_BOOLEAN==kind) {
			if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_boolean(dd, byteByReference, null, member_id))) {
				return null;
			} else {
			...
			if(Number[].class.equals(tgt)) {
				...
				if(0 == RTICLibrary.INSTANCE.DDS_DoubleSeq_ensure_length(seq.getPointer(), 0, DOUBLE_SEQUENCE_EXTENT)) {
					throw new RuntimeException("Unable to ensure_length");
				}
				if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_double_seq(dd, seq.getPointer(), null, member_id))) {
					return new Number[0];
				}
				
				int length = RTICLibrary.INSTANCE.DDS_DoubleSeq_get_length(seq.getPointer());
				if(null == numbers || length > numbers.length) {
					numbers = new Number[length];
				}
				
				for(int i = 0; i < length; i++) {
					numbers[i] = RTICLibrary.INSTANCE.DDS_DoubleSeq_get(seq.getPointer(), i);
				}
				return numbers;
			} else if(byte[].class.equals(tgt)) {
				RTICLibrary.DDS_Sequence seq = readOctetSequence;

				if(0 == RTICLibrary.INSTANCE.DDS_OctetSeq_ensure_length(seq.getPointer(), 0, OCTET_SEQUENCE_EXTENT)) {
					throw new RuntimeException("Unable to ensure_length");
				}
				if(checkReturnCodeForNoData(RTICLibrary.INSTANCE.DDS_DynamicData_get_octet_seq(dd, seq.getPointer(), null, member_id))) {
					return new byte[0];
				}
				
				int length = RTICLibrary.INSTANCE.DDS_OctetSeq_get_length(seq.getPointer());
				if(null == octets || octets.length < length) {
					octets = new byte[length];
				}
				for(int i = 0; i < length; i++) {
					octets[i] = RTICLibrary.INSTANCE.DDS_OctetSeq_get(seq.getPointer(), i);
				}
				return octets;
			} else {
				return null;
	...
	public final void set(String name, Pointer dd, Object o, int member_id, int kind, Pointer tc) {
		if(null == o) {
			return;
		}
		...
		if(RTICLibrary.DDS_TK_STRING==kind) {
			if(o instanceof Enum) {
				o = ((Enum<?>)o).getDeclaringClass().getName()+"."+((Enum<?>)o).name();
			}
			...
			if(o instanceof Date) {
				checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_longlong(dd, null, member_id, ((Date)o).getTime()));
			} else {
				checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_longlong(dd, null, member_id, (Long) o));
			}
		} else if(RTICLibrary.DDS_TK_LONG==kind) {
			checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_long(dd, null, member_id, (Integer) o));
		} else if(RTICLibrary.DDS_TK_SHORT==kind) {
			checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_short(dd, null, member_id, (Short) o));
		} else if(RTICLibrary.DDS_TK_CHAR==kind) {
			checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_octet(dd, null, member_id, (Byte) o));
		} else if(RTICLibrary.DDS_TK_DOUBLE==kind) {
			if( o instanceof Double) {
				checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_double(dd, null, member_id, (Double) o));
			} else if ( o instanceof Number ) {
				checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_double(dd, null, member_id, ((Number)o).doubleValue()));
			}
		} else if(RTICLibrary.DDS_TK_FLOAT==kind) {
			checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_float(dd, null, member_id, (Float) o));
		} else if(RTICLibrary.DDS_TK_BOOLEAN==kind) {
			checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_boolean(dd, null, member_id, (Boolean) o));
		} else if(RTICLibrary.DDS_TK_ARRAY==kind) {
			Pointer tc_ = RTICLibrary.INSTANCE.DDS_TypeCode_content_type(tc, writeException);
			checkWriteException();
			if(null == tc_) {
				throw new RuntimeException("Unable to get content_type");
			}
			
//			int componentKind = RTICLibrary.INSTANCE.DDS_TypeCode_kind(tc_, writeException);
//			checkWriteException();
			...
			if(RTICLibrary.DDS_TK_DOUBLE==componentKind) {
				...
				checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_set_double_seq(dd, null, member_id, seq.getPointer()));
			} else if(RTICLibrary.DDS_TK_OCTET==componentKind) {
				byte[] b = (byte[]) o;
				if(b.length > OCTET_SEQUENCE_EXTENT) {
					throw new RuntimeException("Cannot specify " + b.length + " bytes in this octet sequence member " + name + " because the max_extent is " + OCTET_SEQUENCE_EXTENT);
				}
...
//			RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_STRING);
			Pointer p =  RTICLibrary.INSTANCE.DDS_TypeCodeFactory_create_string_tc(factory, 4096, writeException);
			checkWriteException();
			return p;
		} else if(Long.class.equals(cls) || Date.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_LONGLONG);
		} else if(Integer.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_LONG);
		} else if(int.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_LONG);
		} else if(Short.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_SHORT);
		} else if(Byte.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_CHAR);
		} else if(Double.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_DOUBLE);
		} else if(Float.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_FLOAT);
		} else if(Boolean.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_BOOLEAN);
		} else if(Enum.class.isAssignableFrom(cls)) {
			Pointer p = RTICLibrary.INSTANCE.DDS_TypeCodeFactory_create_string_tc(factory, 1024, writeException);
			checkWriteException();
			return p;
		} else if(byte.class.equals(cls)) {
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_OCTET);
		} else if(Number.class.isAssignableFrom(cls)) {
			// Un anticipated number type transferred as double
			return RTICLibrary.INSTANCE.DDS_TypeCodeFactory_get_primitive_tc(factory, RTICLibrary.DDS_TK_DOUBLE);
		} else if(cls.isArray()) {
			if(Number.class.equals(cls.getComponentType())) {
				Pointer p =  RTICLibrary.INSTANCE.DDS_TypeCodeFactory_create_sequence_tc(factory, DOUBLE_SEQUENCE_EXTENT, primitive(cls.getComponentType()), writeException);
				checkWriteException();
				return p;
			} else if(byte.class.equals(cls.getComponentType())) {
				Pointer p =  RTICLibrary.INSTANCE.DDS_TypeCodeFactory_create_sequence_tc(factory, OCTET_SEQUENCE_EXTENT, primitive(cls.getComponentType()), writeException);
				checkWriteException();
				return p;
			} else if(String.class.equals(cls.getComponentType())) {
				Pointer p = RTICLibrary.INSTANCE.DDS_TypeCodeFactory_create_string_tc(factory, STRING_EXTENT, writeException);
				checkWriteException();
				return p;
	...
	public Pointer buildTypeCode(Class<?> iface, GetterPopulation getters) {
		...
		if(getters.getSparse()) {
			tc_comp = RTICLibrary.INSTANCE.DDS_TypeCodeFactory_create_sparse_tc(factory, RTICLibrary.INSTANCE.DDS_String_dup(iface.getName()), 0, Pointer.NULL, writeException);
		} else {
			DDS_Sequence sequence = new DDS_Sequence();
			RTICLibrary.INSTANCE.DDS_StringSeq_initialize(sequence.getPointer());
			sequence.read();
			tc_comp = RTICLibrary.INSTANCE.DDS_TypeCodeFactory_create_struct_tc(factory, RTICLibrary.INSTANCE.DDS_String_dup(iface.getName()), sequence.getPointer(), writeException);
		}
		...
		for(Getter get : getters.getters) {
			...
			if(tc != null) {
				...
				if(getters.getSparse()) {
					get.setIndex(RTICLibrary.INSTANCE.DDS_TypeCode_add_member(tc_comp, RTICLibrary.INSTANCE.DDS_String_dup(get.getName()), i++, tc, RTICLibrary.DDS_TYPECODE_NONKEY_MEMBER, writeException));
					checkWriteException();
					get.setMemberId(i-1);
					get.setKind(RTICLibrary.INSTANCE.DDS_TypeCode_kind(tc, writeException));
					checkWriteException();
					
//					log.trace(get.getName() + " idx="+get.getIndex() + " idx="+(i-1) + " iface="+iface);
					
				} else {
					get.setIndex(RTICLibrary.INSTANCE.DDS_TypeCode_add_member(tc_comp, RTICLibrary.INSTANCE.DDS_String_dup(get.getName()), i++, tc, get.isKey() ? RTICLibrary.DDS_TYPECODE_KEY_MEMBER : RTICLibrary.DDS_TYPECODE_NONKEY_MEMBER, writeException));
					checkWriteException();
					get.setMemberId(i-1);
					get.setKind(RTICLibrary.INSTANCE.DDS_TypeCode_kind(tc, writeException));
					checkWriteException();
...
//				int idx = sparseTC.add_member(get.getName(), i++, tc, TypeCode.NONKEY_MEMBER, PUBLIC_MEMBER.VALUE, false, TypeCode.NOT_BITFIELD);
	...
	public void set(IdentifiableUpdate<?> update, Pointer dds_update) {
		checkReturnCode(RTICLibrary.INSTANCE.DDS_DynamicData_clear_all_members(dds_update));
...
//			if(null != m.getDeclaringClass().getAnnotation(Persistent.class)) {
			Persistent p = m.getAnnotation(Persistent.class);
				if(null != p) {
					...
					} else if(m.getName().startsWith("is") && m.getParameterTypes().length == 0 && m.getReturnType().equals(Boolean.class)) {
						get.getGetters().add(new GetterImpl(m, p));
					}
...
//			}
	...
	public final Identifier identifier(String fieldName) {
		...
		if(identifier.containsKey(fieldName)) {
			...
			if(idx < 0) {
				throw new IllegalArgumentException("Not a valid fieldName:" +fieldName);
			}
			try {
				i = (Identifier) Class.forName(fieldName.substring(0, idx)).getField(fieldName.substring(idx+1, fieldName.length())).get(null);
