...
public class WaveformView extends android.view.View implements WaveformSourceListener, WaveformCanvas, WaveformCanvas.Extent, WaveformRepresentation {
...
//		renderingBitmap = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);

//		if(USE_A_CANVAS) {
//			Canvas c = new Canvas();
//			c.setBitmap(bitmap[0]);
...
//				drawingBitmap.copyPixelsToBuffer(xferBuffer);
//				renderingBitmap.copyPixelsFromBuffer(xferBuffer);
			}
...
//			this.dct_source = null;
	...
	public void drawLine(int color, int x0, int y0, int x1, int y1) {
		...
		int spany = y1-y0;
		for(int x = x0; x <= x1; x++) {
			int y = (int) (1.0f * (x - x0) / spanx * spany + y0) - 1;
//			Log.d(WaveformView.class.getName(), "x="+x+",y="+y+",width="+drawingBitmap.getWidth()+",height="+drawingBitmap.getHeight()+",x0="+x0+",x1="+x1+",y0="+y0+",y1="+y1);
