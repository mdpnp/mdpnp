...
public class ScenarioPanel extends Composite implements Editor<ScenarioProxy> {
	...
	public final static String SCN_LAST_ACTION_REAPPROVED = "reapproved"; // after the second and successive approvals
	...
	private enum UserRole {
		Administrator, RegisteredUser, AnonymousUser
	}
	...
	private List<ScenarioProxy> scenarioSearchResultsList = null;// list with the scenarios of latest search
	private int positionInSearchResultsList; // position of this scenario in the scenario list

	public List<ScenarioProxy> getScenarioSearchResultsList() {
		return scenarioSearchResultsList;
	}

	public void setScenarioSearchResultsList(List<ScenarioProxy> scenarioList) {
		this.scenarioSearchResultsList = scenarioList;
	}

	public int getPositionInSearchResultsList() {
		return positionInSearchResultsList;
	}

	public void setPositionInSearchResultsList(int position) {
		this.positionInSearchResultsList = position;
	}

	/**
	 * Check if its possible to edit this scenario information and updates the
	 * flag;
	 */
	private void checkEditable() {
		...
		if (!(currentScenario.getStatus().equals(SCN_STATUS_UNSUBMITTED)
				|| currentScenario.getStatus().equals(SCN_STATUS_UNLOCKED_PRE) || currentScenario
				.getStatus().equals(SCN_STATUS_UNLOCKED_POST)))
			editable = false;
		// if unsumbitted, only the creator can edit
		if (currentScenario.getStatus().equals(SCN_STATUS_UNSUBMITTED)
				&& !userEmail.equals(currentScenario.getSubmitter()))
			editable = false;

		// if unlocked_pre or unlocked_post, only owner of lock can edit.
		if ((currentScenario.getStatus().equals(SCN_STATUS_UNLOCKED_PRE) || currentScenario
				.getStatus().equals(SCN_STATUS_UNLOCKED_POST))
				&& !currentScenario.getLockOwner().equals(userEmail))
			editable = false;
	...
	interface Driver extends
			RequestFactoryEditorDriver<ScenarioProxy, ScenarioPanel> {
	...
	/*Legacy code: This is an interface (to implement on Clinical_scenarios, to redirect users to other panels after an action
	public interface RedirectHandler {
		void onClickSubmit();
	}
	
	private RedirectHandler redirectHandler;//to redirect after some events
	...
	*/
	 ...
	 */
	private static final String[] hazardExpected = new String[] { "Unknown",
			"Expected", "Unexpected" };
	...
	private int getHazardExpectedIndex(String word) {
		for (int i = 0; i < hazardExpected.length; i++) {
			if (word.equals(hazardExpected[i]))
				return i;
		}
		return -1;
	}

	// private static final String[] hazardSeverity = new String[] {"Unknown",
	// "Mild", "Moderate", "Severe", "Life Threatening", "Fatal"};
	/**
	 * List of different types/degree of hazards
	 */
	private static final String[] hazardSeverity = new String[] { "Unknown",
			"No Harm", "Mild", "Moderate", "Severe", "Permanent Harm",
	...
	private int getHazardSeverityIndex(String word) {
		for (int i = 0; i < hazardSeverity.length; i++) {
			if (word.equals(hazardSeverity[i]))
				return i;
		}
		return -1;
	}
	...
	// private static final String[] testCasesHeader = new String[] {"Id",
	// "Description", "Step/Order", "Author", "Requirements", "Configuration",
	// "Remarks", "Summary"};
	 ...
	 * 
	 * @param isDrawNew
	 *            indicates if we are drawing a new/empty table or we are going
	 *            to populate it with data from an existing scenario.
	 */
	private final void buildCliniciansTable(boolean isDrawNew) {
		cliniciansTable.removeAllRows();
		if (isDrawNew
				|| currentScenario.getEnvironments().getCliniciansInvolved()
						.isEmpty()) {
			addNewClinicianRow("");
		} else {
			List<String> clinicians = currentScenario.getEnvironments()
					.getCliniciansInvolved();
	...
	private final void buildEnvironmentsTable(boolean isDrawNew) {
		...
		if (isDrawNew
				|| currentScenario.getEnvironments().getClinicalEnvironments()
						.isEmpty()) {
			addNewEnvironmentRow("");
		} else {
			List<String> environments = currentScenario.getEnvironments()
					.getClinicalEnvironments();
	...
	// labels for the HEADERS row of the Devices tab table. we use the following
	// static block to initialize some of
	...
	private static Label labelEquipmentRosettaId = new Label(
			"Rosetta/Unique ID");
	private static Label labelEquipmentProblemDescrp = new Label(
			"Problem Description");
	private static Label labelEquipmentTrainingProblem = new Label(
			"Training Related Problem");
	private static Label labelEquipmentInstructionsProblem = new Label(
			"Instructions Related Problem");
	private static Label labelEquipmentConfusingUsage = new Label(
			"Confusing device Usage");
	private static Label labelEquipmentSoftWProblem = new Label(
			"Software related problem");
	private static Label labelEquipmentHardWProblem = new Label(
			"Hardware related problem");

	{
		// initialize the labels
		labelEquipmentDeviceType
				.setTitle("Type of device, such as \"ventilator\" or \"Infusion pump\" ");
		labelEquipmentManufacturer
				.setTitle("Manufacturer/company or device provider");
		labelEquipmentModel.setTitle("Model of the device");
		labelEquipmentRosettaId.setTitle("Unique identifier of this device");
		labelEquipmentProblemDescrp
				.setTitle("Brief description of the device malfunctioning");
		labelEquipmentTrainingProblem
				.setTitle("A gap, lack of or inadequate training with this device contributted to the problem");
		labelEquipmentInstructionsProblem
				.setTitle("Instructions or documentation for this device were confusing, unavailable or outdated"
						+ "contributting to the problem");
		labelEquipmentConfusingUsage
				.setTitle("Confusing interfaces, settings, controls, o general usage procedure of the device contributted to the problem");
		labelEquipmentSoftWProblem
				.setTitle("A software problem contributed to the problem");
		labelEquipmentHardWProblem
				.setTitle("A hardware problem contributed to the problem");
	}
	 ...
	 * @param isDrawNew
	 *            indicates if we are drawing a new/empty table or we are going
	 *            to populate it with data from the scenario.
	 */
	private final void buildEquipmentTable(boolean isDrawNew) {
		...
		equipmentTable.setWidget(0, EQUIPMENT_DEVICETYPE_COL,
				labelEquipmentDeviceType);
		equipmentTable.setWidget(0, EQUIPMENT_MANUFACTURER_COL,
				labelEquipmentManufacturer);
		equipmentTable.setWidget(0, EQUIPMENT_MODEL_COL, labelEquipmentModel);
		equipmentTable.setWidget(0, EQUIPMENT_ROSSETAID_COL,
				labelEquipmentRosettaId);
		equipmentTable.setWidget(0, EQUIPMENT_EXPLANATION_COL,
				labelEquipmentProblemDescrp);
		equipmentTable.setWidget(0, EQUIPMENT_TRAINING_PROBLEM_COL,
				labelEquipmentTrainingProblem);
		equipmentTable.setWidget(0, EQUIPMENT_INSTRUCTION_PROBLEM_COL,
				labelEquipmentInstructionsProblem);
		equipmentTable.setWidget(0, EQUIPMENT_CONFUSING_USAGE_COL,
				labelEquipmentConfusingUsage);
		equipmentTable.setWidget(0, EQUIPMENT_SW_PROBLEM_COL,
				labelEquipmentSoftWProblem);
		equipmentTable.setWidget(0, EQUIPMENT_HW_PROBLEM_COL,
				labelEquipmentHardWProblem);
		...
		if (isDrawNew || currentScenario.getEquipment().getEntries().isEmpty()) {
			...
			// populate the table w/ the data from the equipment list of the
			// scenario
			List<EquipmentEntryProxy> eqEntries = currentScenario
					.getEquipment().getEntries();
	...
	// Labels for the header row of the HAZARDS table. We are using the static
	// block
	// to initialize some of the labels properties, such as the title/tooltip of
	// them
	private final static Label labelHazardDescription = new Label("Description");
	private final static Label labelHazardFactors = new Label(
...

	{
		...
		labelHazardDescription
				.setTitle("Describe the risk. Make sure to point out if this risk results in death, is life threatening, requires inpatient hospitalization or prolongation of existing hospitalization, results in persistent or significant disability/incapacity, is a congenital anomaly/birth defect");
		labelHazardFactors
				.setTitle("Determine which factors are contributing to the risk described below. Examples may be a clinician, a specific device, an aspect of the clinical envirnomnet, etc.");
		...
		labelHazardsExpected.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				MyDialog md = new MyDialog(
						"Unexpected: Risk is not consistent with the any of risks known (from a manual, label, protocol, instructions, brochure, etc) in the Current State. If these documents are not required or available, the risk is unexpected if specificity or severity is not consistent with the risk information described in the protocol, or it is more severe to the specified risk",
	...
	private final void buildHazardsTable(boolean isDrawNew) {
		...
		hazardsTable.setWidget(0, HAZARDS_DESCRIPTION_COL,
				labelHazardDescription);
		...
		if (isDrawNew || currentScenario.getHazards().getEntries().isEmpty())
			...
			// populate the table
			List<HazardsEntryProxy> hazards = currentScenario.getHazards()
					.getEntries();
			for (int i = 0; i < hazards.size(); i++) {
				HazardsEntryProxy hep = (HazardsEntryProxy) hazards.get(i);
				addNewHazardTableRow(hep.getDescription(), hep.getFactors(),
						hep.getExpected(), hep.getSeverity());
			}
		}
	...
	private final void buildReferencesTable(boolean isDrawNew) {
		...
		// XXX currentScenario.getReferences()!=null because was added after all
		// the other fields
		if (isDrawNew
				|| currentScenario.getReferences() == null
				|| currentScenario.getReferences().getLinkedRefenrences()
						.isEmpty()) {
			addNewLinkedReference();
		} else {
			// populate the table
			for (String ref : currentScenario.getReferences()
					.getLinkedRefenrences()) {
		...
		if (isDrawnNew // ||userRole==UserRole.AnonymousUser
				|| !(currentScenario.getStatus().equals(SCN_STATUS_APPROVED)
						|| currentScenario.getStatus().equals(
								SCN_STATUS_MODIFIED) || currentScenario
						.getStatus().equals(SCN_STATUS_UNLOCKED_POST))) {
			// XXX needs to be just approved or unlocked too? WHO can tag? Any
			// user?
			tagsLabel
					.setText("The scenario needs to be APPROVED before any keyword can be tagged");
		} else {
				...
				tagRequest.findAll().to(new Receiver<List<TagProxy>>() {
					...
					public void onSuccess(List<TagProxy> response) {
						if (userRole == UserRole.Administrator)
							...
							tagsLabel
									.setText("Only Repository Administrators can tag a scenario");

						Set<String> associatedTags = currentScenario
								.getAssociatedTags().getAssociatedTagNames();
						int indexRow = 0;
						int indexCol = 0;
						// sort the response w/ the appropriate comparator
						// before traversing
	...
	/**
	 * Checks if we need to persist the equipment list.
	 * <p>
	 * It deletes all rows of the list of equipment associate with the current
	 * scenario and adds all the data that is in the table.
	 * 
	 * @param rc
	 *            ScenarioRequest
	 */
	private void checkEquipmentListForPersistence(ScenarioRequest rc) {
		...
		currentScenario.getEquipment().getEntries().clear();// clear equipment
															// list entries. We
															// will re-populate
		for (int row = 1; row < equipmentTable.getRowCount(); row++) {// Row 0
																		// is
																		// HEADERS

			Widget wDevType = equipmentTable.getWidget(row,
					EQUIPMENT_DEVICETYPE_COL);
			Widget wManu = equipmentTable.getWidget(row,
					EQUIPMENT_MANUFACTURER_COL);
			Widget wModel = equipmentTable.getWidget(row, EQUIPMENT_MODEL_COL);
			Widget wRoss = equipmentTable.getWidget(row,
					EQUIPMENT_ROSSETAID_COL);
			Widget wExplanation = equipmentTable.getWidget(row,
					EQUIPMENT_EXPLANATION_COL);
			Widget wTraining = equipmentTable.getWidget(row,
					EQUIPMENT_TRAINING_PROBLEM_COL);
			Widget wInstruction = equipmentTable.getWidget(row,
					EQUIPMENT_INSTRUCTION_PROBLEM_COL);
			Widget wUsage = equipmentTable.getWidget(row,
					EQUIPMENT_CONFUSING_USAGE_COL);
			Widget wSoftProblem = equipmentTable.getWidget(row,
					EQUIPMENT_SW_PROBLEM_COL);
			Widget wHardProblem = equipmentTable.getWidget(row,
					EQUIPMENT_HW_PROBLEM_COL);
			...
			if (wExplanation instanceof TextArea) {
				...
				if (!text.equals("")) {
					eep.setProblemDescription(text);
					isAdding = true;
				}
			}
			...
			eep.setInstructionsRelatedProblem(((CheckBox) wInstruction)
					.getValue());
	...
	/**
	 * Checks if we need to persist the hazards list
	 * <p>
	 * 
	 * @param rc
	 *            ScenarioRequest
	...
	private void checkHazardsListForPersistence(ScenarioRequest rc) {
		currentScenario.getHazards().getEntries().clear();// clean and
															// re-populate
		// for(int row =2; row<hazardsTable.getRowCount();row++){//row 0
		// headers; row 1 description
		for (int row = 1; row < hazardsTable.getRowCount(); row++) {// row 0
																	// headers+description

			Widget wDescription = hazardsTable.getWidget(row,
					HAZARDS_DESCRIPTION_COL);
			Widget wFactor = hazardsTable.getWidget(row, HAZARDS_FACTORS_COL);
			Widget wExpected = hazardsTable
					.getWidget(row, HAZARDS_EXPECTED_COL);
			Widget wSeverity = hazardsTable
					.getWidget(row, HAZARDS_SEVERITY_COL);
			...
			// We assume that makes no sense having no text for
			// DEscription/factor and trying to persist
			// values for risk/severity (there would be no hazard
			// identification)
			if (wDescription instanceof TextArea) {
				text = ((TextArea) wDescription).getText().trim();
				if (!text.equals("")) {
					hep.setDescription(text);
					isAdding = true;
				}
			}
	...
	private void checkCliniciansListForPersistence(/* ScenarioRequest scn */) {
		...
		for (int row = 0; row < cliniciansTable.getRowCount(); row++) {
			Widget wClinician = cliniciansTable.getWidget(row,
					CLINICIANS_TYPE_COL);
	...
	private void checkEnvironmentsListForPersistence() {
		...
		for (int row = 0; row < environmentsTable.getRowCount(); row++) {
			Widget wEnvironment = environmentsTable.getWidget(row,
					ENVIRONMENT_TYPE_COL);
	...
	/**
	 * Checks the content of the user interface fields and updates the
	 * information in the scenario entity that we are going to use for
	 * persistence
	 * 
	 * @param scnReq
	 *            a ScenarioRequest object is needed to create some of the
	 *            components of our list-type parameters (list of hazards, list
	 *            of equipment).
	 */
	 ...
	 * <p>
	 * 1- The TextArea fields are associated automatically w/ the corresponding
	 * attributes in the Scenario object (Background, proposed solution and
	 * Benefits & risks)
	 * <p>
	 * 2- Save the list of equipment
	 * <p>
	 * 3- Save the list of hazards
	 * <p>
	 * 4- Save Clinicians and Environment List
	 * <p>
	 * Persist the scenario entity with all its associated values
	 */
	private void save() {
		if (!editable)
			return; // Ticket-144 Mutual exclusion / persistence permission
		if (isEmptyScenario())
			return;// Ticket-81 Don't persist empty Scn
		...
		rc.persist()
				...
				.with("equipment", "hazards", "environments", "references",
						"acknowledgers", "associatedTags")
				...
				.to(new Receiver<ScenarioProxy>() {
					...
					public void onSuccess(ScenarioProxy response) {
						...
						if( scenarioSearchResultsList!=null)//new scenario
							scenarioSearchResultsList.set(positionInSearchResultsList, currentScenario);// TICKET-145
						setCurrentScenario(response);
						// logger.info("AFTER:"+currentScenario.getTitle());
						logger.info("SAVED " + currentScenario.toString()
								+ " @ " + (new Date()));
					}
	...
	/**
	 * Selects the first tab of the form.
	 * <p>
	 * XXX Careful with circular calls
	 * selectTab->save->setScenario->selectTab(0)
	 */
	public void selectFirstTab() {
		boolean tempEditable = editable;
		editable = false;// prevent save() call due to SelectionHandler and so
							// funny behavior
		/**
		 * This editable = false condition is here to prevent the following
		 * behavior: In case that we are in an editable scenario, in a tab
		 * different from the first one, when we move to a different scenario
		 * this selectFirstTab() method is called. Since we have a
		 * SelectionHandler attached to the tabPanel, this method will fire the
		 * handler event.
		 * 
		 * The effect is that we see the new selected scenario briefly and the
		 * it changes to the old one, because the save() method from the
		 * tabSelection also performs a setCurrentScenario, with the information
	 ...
	 * Should save the current scenario. Depending on the tab we are some
	...
	public ScenarioPanel(final ScenarioRequestFactory scenarioRequestFactory) {
		...
		// check user role
		// XXX We should use dependency injection to provide this information
		// instead of doing a request
		if (userInfoRequestFactory != null) {
			final EventBus eventBus = new SimpleEventBus();
			userInfoRequestFactory.initialize(eventBus);

			UserInfoRequest userInfoRequest = userInfoRequestFactory
					.userInfoRequest();
			userInfoRequest
					.findCurrentUserInfo(Window.Location.getHref(), false)
					.with("loginURL").to(new Receiver<UserInfoProxy>() {
...
//		lockButton.setTitle("Locks/Unlocks the scenario to allow modification of its content");
	...
	/**
	 * Disables the "editable" property of a Scenario. We don't see the buttons
	 * to save, we can't interact w/ the panels
	 */
	...
	/**
	 * builds the tables of the different panels and fills them with the
	...
	public void setCurrentScenario(ScenarioProxy currentScenario) {
		...
		if (null == currentScenario) {
			// speed up the process of configuring the proper interface
			if (tabPanel.getWidgetCount() > APPRV_SCN_TAB_POS) {
				tabPanel.remove(APPRV_SCN_TAB_POS);
				selectFirstTab();
			}
			prevScenarioButton.setVisible(false);
			nextScenarioButton.setVisible(false);
			// -----------------------------------
					...
					.to(new Receiver<ScenarioProxy>() {
						...
						public void onSuccess(ScenarioProxy response) {
							if (response == null) {
								...
								titleEditor
										.setText("Please sign in to create new Clinical Scenarios");
							}

							logger.info("" + response.getBackground());
							ScenarioRequest context = scenarioRequestFactory
									.scenarioRequest();
							ScenarioProxy currentScenario = context
									.edit(response);
							driver.edit(currentScenario, context);
							ScenarioPanel.this.currentScenario = currentScenario;

							// after the Entity has been successfully created,
							// we populate the widgets w/ the entity info and
							// draw it
							scenarioSearchResultsList = null;// TICKET-145
							positionInSearchResultsList = 0;
							configureComponents();
							buildTabsTables(true);// Ticket-175 build tables
													// AFTER "editable" has been
													// checked on
													// configureComponents()
							uniqueId.setText("");
						}
			...
			/*
			 * Call edit() so we can change the currentScenario and set the
			 * value of its fields
			 */
			...
			if (currentScenario.getId() != null)
				uniqueId.setText("Scenario Unique ID: "
						+ String.valueOf(currentScenario.getId()));
			configureComponents();// XXX Maybe configureComponents() should be
									// called inside buildTabsTables
			buildTabsTables(false);// Ticket-175 build tables AFTER "editable"
									// has been checked on configureComponents()
		}

		// select first tab
		// tabPanel.selectTab(0); DANGER DANGER DANGER
		// XXX CAREFUL!!! Selecting a tab here triggers again the associated tab
		// ClickHandler, that calls this very same method
		// and we have a circular call and therefore an infinite loop.

	}

	/**
	 * Configures the different components (status label, buttons, etc.)
	 * according to scenario status, user role...
	 */
	private void configureComponents() {
		...
		// 2- submit button
		if (currentScenario.getStatus().equals(SCN_STATUS_UNSUBMITTED)
				&& currentScenario.getSubmitter().equals(userEmail)) {
			submitButton.setVisible(true);
		} else {
			submitButton.setVisible(false);
		}
		...
		if (userRole == UserRole.Administrator) {
			if (currentScenario.getStatus().equals(SCN_STATUS_UNSUBMITTED)
					|| currentScenario.getStatus().equals(SCN_STATUS_REJECTED)) {
				lockButton.setVisible(false);
			} else {
				lockButton.setVisible(true);
				if (currentScenario.getStatus().equals(SCN_STATUS_UNLOCKED_PRE)
						|| currentScenario.getStatus().equals(
								SCN_STATUS_UNLOCKED_POST)) {
		...
		if (userRole == UserRole.Administrator && (// currentScenario.getStatus().equals(SCN_STATUS_SUBMITTED)
				...
				currentScenario.getStatus().equals(SCN_STATUS_UNLOCKED_PRE) || currentScenario
						.getStatus().equals(SCN_STATUS_UNLOCKED_POST))) {
			if (tabPanel.getWidgetCount() <= APPRV_SCN_TAB_POS)// add Feedback
																// tab if is not
																// there
		...
		// If this user has previously acknowledged the scenario, we hide the
		// button
		...
		if (userRole != UserRole.AnonymousUser
				&& (currentScenario.getStatus().equals(SCN_STATUS_APPROVED)
						|| currentScenario.getStatus().equals(
								SCN_STATUS_UNLOCKED_POST) || currentScenario
						.getStatus().equals(SCN_STATUS_MODIFIED))) {
			if (ackEdgers == null) {
				// can be null because is a new field, and the previous entities
				// on the GAE don't have this field
				ackButton.setVisible(true);// because is obvious that this user
											// didn't ack the scenario
			} else {
				ackButton.setVisible(!currentScenario.getAcknowledgers()
						.getAcknowledgersIDs().contains(userId));
			}
			// ackButton.setVisible(currentScenario.getAcknowledgers().contains(userId));
			// XXX display the above information only if the current scenarios
			// already has acks?

		} else {
			ackButton.setVisible(false);// either unregistered user or scn not
										// approved
		}

		if (ackEdgers != null
				&& currentScenario.getAcknowledgers().getAcknowledgersIDs()
						.size() > 0) {
			// we show ack feedback in all cases
			ackLabel.setVisible(true);
			ackLabel.setText(currentScenario.getAcknowledgers()
					.getAcknowledgersIDs().size()
					+ " Users have acknowledged this scenario");
			ackLabel.setTitle(currentScenario.getAcknowledgers()
					.getAcknowledgersIDs().size()
					+ " Users confirmed the importance and genuinity of this scenario"
		...
		// XXX This condition could be approved (or post approved) + no
		// unregistered user
		...
		// 7- Buttons to navigate (traverse, iterate) to the next/previous
		// scenario
		if (scenarioSearchResultsList == null
				|| scenarioSearchResultsList.size() == 0) {
			// this is the case for a NEW Scenario
			prevScenarioButton.setVisible(false);
			nextScenarioButton.setVisible(false);
		} else {
			prevScenarioButton.setVisible(true);
			nextScenarioButton.setVisible(true);

			prevScenarioButton.setEnabled(true);
			nextScenarioButton.setEnabled(true);
			if (positionInSearchResultsList == 0) {
				prevScenarioButton.setEnabled(false);
			}
			if (positionInSearchResultsList == scenarioSearchResultsList.size() - 1) {
	...
	private static class ClinicianSuggestOracle extends MultiWordSuggestOracle {
		private static String[] values = new String[] { "Allergist",
				"Anesthesiologist", "Cardiologist", "Chief Nurse",
				"Clinical Staff", "CNA - Certified Nurse Assistant",
				"Cosmetic Surgeon", "Critical Care Nurse",
				"CRNA - Certified Registered Nurse Asst", "Dentist",
				"Dermatologist", "Emergency Medicine Doctor",
				"Endocrinologist", "Epidemiologist", "Gastrologist",
				"General Practitioner", "Geriatrics Specialist",
				"Gynecologist", "Hematologist", "HHN - Home Health Nurse",
				"Hospitalist", "Infectious Disease Nurse",
				"Labor-Delivery Nurse", "LPN - Licensed Practical Nurse",
				"LVN - Licensed Vocational Nurse", "Medical Assistant",
				"Medical Emergency Team", "Neonatologist", "Neurologist",
				"Nurse", "Nurse Assistant", "Occupational Health Nurse",
				"Oncologist", "ORRN - Operating Room Registered Nurse",
				"Pathologist", "Physician", "Radiologist",
				"Respiratory Therapist", "Rheumatologist",
				"RN - Registered Nurse", "RRT", "Surgeon", "Toxicologist",
				"Urologist", "X-Ray Technician"
	...
	private static class EnvironmentSuggestOracle extends
			MultiWordSuggestOracle {
		private static String[] values = new String[] {
				"Acute assessment unit", "Ambulatory wing",
				"Birthing Room/LDR Room/LDRP Room", "Breast screening unit",
				"Burn Care Unit", "Cafeteria", "CCU-cardiac care unit",
				"Delivery room", "Discharge lounge", "Discharge unit",
				"ENT-Ear nose and throat", "ER-emergency room", "Geriatrics",
				"Hospital room", "ICU-intensive care unit", "Lab/pathology",
				"Maternity wards", "Neonatal unit", "Nurse station",
				"On-call room", "OR-operating room", "Patient bay",
				"Pediatrics", "Physical therapy/rehab dept",
				"Post surgical care unit", "Psychiatric ward",
				"Radiology/imaging", "ICU-intensive care unit", "Renal unit",
				"Telemetry", "Transport", "Trauma center", "Ultrasound Unit",
				"Volunteers room", "Waiting room", "Wound Care Unit", };
	...
	private void addNewEquipmentRow() {
		...
		for (int j = 0; j < EQUIPMENT_EXPLANATION_COL; j++) {// add four text
																// boxes
		...
		for (int j = EQUIPMENT_TRAINING_PROBLEM_COL; j < EQUIPMENT_DElETEBUTTON_COL; j++) {// add
																							// four
																							// check
																							// boxes
			CheckBox cb = new CheckBox();
		...
		if (editable)
			equipmentTable.setWidget(rows, EQUIPMENT_DElETEBUTTON_COL,
					deleteButton);
	...
	private void addNewEquipmentRow(EquipmentEntryProxy eep) {
		...
		equipmentTable
				.setWidget(rows, EQUIPMENT_MANUFACTURER_COL, manufTextBox);
		...
		equipmentTable.setWidget(rows, EQUIPMENT_EXPLANATION_COL,
				explanationTextArea);
		...
		equipmentTable.setWidget(rows, EQUIPMENT_TRAINING_PROBLEM_COL,
				trainingCheckBox);
		...
		equipmentTable.setWidget(rows, EQUIPMENT_INSTRUCTION_PROBLEM_COL,
		...
		equipmentTable.setWidget(rows, EQUIPMENT_CONFUSING_USAGE_COL,
				usageCheckBox);
		...
		equipmentTable.setWidget(rows, EQUIPMENT_SW_PROBLEM_COL,
				softProblemCheckBox);
		...
		equipmentTable.setWidget(rows, EQUIPMENT_HW_PROBLEM_COL,
				hardProblemCheckBox);
	...
	void onAddNewHazardClick(ClickEvent click) {
		/**
		 * We use the currentStateEditor to know if the components have been
		 * enabled/disabled for modification and thus know if we should allow to
	...
	private void addNewHazardTableRow() {
		...
		hazardsTable.setWidget(row, HAZARDS_EXPECTED_COL,
				buildListBox(hazardExpected));
		hazardsTable.setWidget(row, HAZARDS_SEVERITY_COL,
				buildListBox(hazardSeverity));
	 ...
	 * @param severity
	 */
	private void addNewHazardTableRow(String description, String factors,
			String expected, String severity) {
		final int row = hazardsTable.getRowCount();
		hazardsTable.insertRow(row);
		final TextArea hazardDescription = new TextArea();
	...
	private void addNewLinkedReference() {
		...
		referencesTable.setWidget(rows, REFERENCE_http_COL,
				new Label("http://"));
		...
		Button deleteButton = new Button("Delete");
		if (editable)
			referencesTable.setWidget(rows, REFERENCE_DELETEBUTTON_COL,
					deleteButton);
		...
		followLink.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				if (!reference.getText().trim().equals(""))
					Window.open("http://" + reference.getText(), "_blank", "");// use
																				// "enabled"
																				// as
																				// third
																				// argument
	...
	void onANCClick(ClickEvent click) {
		...
		if (editable) {// TICKET-110
			...
			cliniciansTable.setWidget(rows, CLINICIANS_DELETEBUTTON_COL,
					deleteButton);
		}
	}
	...
	void onANEClick(ClickEvent click) {
		...
		if (editable) {// TICKET-110
			...
			environmentsTable.setWidget(rows, ENVIRONMENT_DELETEBUTTON_COL,
					deleteButton);
		}
	}
	 ...
	 * 
	 * @param environment
	 *            name of the clinical environment
	 */
	...
	void onCSEClick(ClickEvent click) {
		// MyDialog md = new MyDialog("Current State Example",
	...
	void onPSEClick(ClickEvent click) {
		...
		MyDialog md = new MyDialog(header, example);
		// MyDialog md = new MyDialog("Proposed State Example",
	...
	void onCPClick(ClickEvent click) {
		...
		MyDialog md = new MyDialog(header, example);
		// MyDialog md = new MyDialog("Clinical Processes Example",
		// "The patient is connected to a PCA infusion pump containing morphine sulfate, a large volume infusion pump acting as a carrier line of saline, a pulse oximeter, a non-invasive blood pressure device, a respiration rate monitor and a distributed alarm system. Heart rate and blood pressure, respiration rate, pain score and sedation score are collected as directed by the clinical process for set-up of a PCA pump. An intravenous (IV) line assessment is also completed. The PCA infusion pump, large volume infusion pump, and pulse oximeter are attached to the integrated system. The system queries the hospital information system for the patient's weight, age, and medication list (specifically, whether the patient is receiving sedatives or non-PCA opioids), and searches for a diagnosis of sleep apnea. The system then accesses the physician's orders from the computerized physician order entry system for dosage and rate for the PCA and large volume infusion pump, and verifies the values programmed into the infusion pump. The patient's SpO2 (arterial oxygen saturation measured by pulse oximetry) and respiration rate are monitored continuously.");
		// md.setPopupPosition(click.getClientX(), click.getClientY());
		md.setAutoHideEnabled(true);
		md.showRelativeTo(titleEditor);
	}
	...
	void onADClick(ClickEvent click) {
		...
		MyDialog md = new MyDialog(header, example);
		// MyDialog md = new MyDialog("Algorithm Description Example",
	...
	void onBClick(ClickEvent click) {
		...
		MyDialog md = new MyDialog(header, example);
		// MyDialog md = new MyDialog("Benefits Example",
		// "Add error resistance to the x-ray procedure by eliminating the dependence on the operator (e.g. anesthesia provider) to remember to turn the ventilator back on. Shorten or eliminate the period of apnea, thereby reducing potentially adverse responses to apnea; and Provide the ability to synchronize x-ray exposure with inspiratory hold, without requiring anyone to be present in the x-ray exposure area to manually generate sustained inspiration");
		// md.setPopupPosition(click.getClientX(), click.getClientY());
		md.setAutoHideEnabled(true);
		md.showRelativeTo(titleEditor);
	...
	void onRClick(ClickEvent click) {
		...
		String example = "Example: A synchronization error could lead to x-ray exposure at an incorrect phas tion.";
		MyDialog md = new MyDialog(header, example);
		// MyDialog md = new MyDialog("Risks Example",
		// "A synchronization error could lead to x-ray exposure at an incorrect phase of respiration.");
		// md.setPopupPosition(click.getClientX(), click.getClientY());
		md.setAutoHideEnabled(true);
		md.showRelativeTo(titleEditor);
	}
	...
	public void onClickSubmit(ClickEvent clickEvent) {
		...
		// option 3 we do like 'Approve' or 'Reject' an also send a email
		// message
	...
	// "FileRepository/doDownload?docId=" + dokument.getId(), "", "");
	...
	public void onClickApproveScn(ClickEvent clickEvent) {
		...
		boolean confirm = Window
				.confirm("Are you sure you want to APPROVE this scenario?");
		if (confirm) {
			String subject = "Your scenario \"" + currentScenario.getTitle()
					+ "\" has been approved";
			String message = "Your scenario \""
					+ currentScenario.getTitle()
					+ "\" has been approved by the MD PnP Clinical Scenario Repository Administrators.\n"
					+ "Thank you for your submission.";
			message += "\n\n" + feedback.getText();
			message += "\n The MD PnP Team \n www.mdpnp.org";

			boolean sendEmail = false; // FIXME Mail servce as its own entity w/
										// services
			...
			currentScenario.setStatus(SCN_STATUS_APPROVED);// update entity
															// information
			currentScenario.setLockOwner(null);
			currentScenario.setLastActionUser(userEmail);

			checkScenarioFields(scnReq);// not really that necessary, because it
										// has been updated when clicking the
										// FeedBack tab

			if (sendEmail) {
				scnReq.persistWithNotification(currentScenario.getSubmitter(),
						subject, message)
						.using(currentScenario)
						.with(driver.getPaths())
						.with("equipment", "hazards", "environments",
	...
	public void onClickReturnScn(ClickEvent clickEvent) {
		...
		boolean confirm = Window
				.confirm("Are you sure you want to RETURN this scenario?");
		if (confirm) {
			String subject = "Requested clarification for your scenario \""
					+ currentScenario.getTitle() + "\".";
			String message = "The MD PnP Clinical Scenario Repository Administrators "
					+ "have requested further clarification for your scenario "
					+ currentScenario.getTitle()
					+ ".\n Please see the comments below \n";
			message += "\n \n" + feedback.getText();
			message += "\n The MD PnP Team \n www.mdpnp.org";

			currentScenario.setStatus(SCN_STATUS_UNSUBMITTED);// update entity
																// information
			currentScenario.setLockOwner(null);
			currentScenario.setLastActionTaken(SCN_LAST_ACTION_RETURNED);
			currentScenario.setLastActionUser(userEmail);
			checkScenarioFields(scnReq);// not really that necessary, because it
										// would be updated when clicking the
										// FeedBack tab
	...
	public void onClickRejectScn(ClickEvent clickEvent) {
		...
		boolean confirm = Window
				.confirm("Are you sure you want to REJECT this scenario?");
		if (confirm) {

			currentScenario.setStatus(SCN_STATUS_REJECTED);// update entity
															// information
			currentScenario.setLockOwner(null);
			...
			scnReq.persist()
					...
					.fire(new Receiver<ScenarioProxy>() {
						...
						public void onFailure(ServerFailure error) {
							super.onFailure(error);
							currentScenario.setStatus(SCN_STATUS_SUBMITTED);// update
																			// entity
																			// information
							// setCurrent?
						}
	...
	/**
	 * Returns true if the current scenario has no meaningful information
	 * <p>
	 * 1- no title
	 * <p>
	 * 2- no background
	 * <p>
	 * 3- no/empty hazards list
	 * <p>
	 * 4- no/empty environment list
	 * <p>
	 * 5- no/empty equipment list
	 * <p>
	 * 6- no proposed solution
	 * <p>
	 * 7- no benefits and risks
	 * <p>
	 * 
	 * @return true/false
	 */
	private boolean isEmptyScenario() {
		...
		/*
		 * TODO Maybe all this CHAOS and coded could be spared if we had a flag
		 * Empty, that is true for "new born" scenarios and false for any other
		 * recovered from a query (if its in the GAE, must have been persisted
		 * because it was not empty. --> the constructor and setCurrentScenario
		 * would modifiy the flag
		 * 
		 * This flag would be checked on this method, or its used sustituded
		 * wherever we are using the function isEmptyScenario().
		 * 
		 * To update the value of the flag (from true to false) we could use the
		 * event handlers that detect we changed the textboxes.
		 * 
		 * It could be tricky, since I could edit the title (it would be not
		 * empty) and then delete, leaving the scenario information empty again.
		 */
		...
		if (!benefits.getText().trim().equals("")
				|| !risks.getText().trim().equals(""))
			return false;
		if (!clinicalProcesses.getText().trim().equals("")
				|| !algorithmDescription.getText().trim().equals(""))
			return false;
		// Flextables
		if (hazardsTable.getRowCount() > FIRST_ROW_HAZARDS_EQUIPMENT + 1)
			return false;
		if (equipmentTable.getRowCount() > FIRST_ROW_HAZARDS_EQUIPMENT + 1)
			return false;
		if (cliniciansTable.getRowCount() > FIRST_ROW_CLINICIANS_ENV + 1)
			return false;
		if (environmentsTable.getRowCount() > FIRST_ROW_CLINICIANS_ENV + 1)
			return false;

		/**
		 * We could still have the problem of persisting Scn that have multiple
		 * empty hazards/equipment rows
		 */
		...
		for (int j = 0; j < HAZARDS_EXPECTED_COL; j++) {
			Widget widget = hazardsTable.getWidget(FIRST_ROW_HAZARDS_EQUIPMENT,
					j);
		...
		for (int j = 0; j < EQUIPMENT_EXPLANATION_COL; j++) {
			Widget widget = equipmentTable.getWidget(
					FIRST_ROW_HAZARDS_EQUIPMENT, j);
		...
		Widget wExplain = equipmentTable.getWidget(FIRST_ROW_HAZARDS_EQUIPMENT,
				EQUIPMENT_EXPLANATION_COL);
	...
	public void onClickAcknowledge(ClickEvent clickEvent) {
		...
		boolean confirm = Window
				.confirm("Click Ok to acknowledge the content of this scenario.");
		if (confirm) {
			Set<String> ackedgers = currentScenario.getAcknowledgers()
					.getAcknowledgersIDs() == null ? new HashSet<String>()
					: currentScenario.getAcknowledgers().getAcknowledgersIDs();
	...
	public void onClickSaveTagsButton(ClickEvent clickEvent) {
		...
		for (int i = 0; i < tagsAssociatedTable.getRowCount(); i++) {
			for (int j = 0; j < (TAGS_TAB_NUM_COL * 2); j += 2) {
				...
				if (tagsAssociatedTable.isCellPresent(i, j)) {// make sure the
																// column exists
					Widget wCheck = tagsAssociatedTable.getWidget(i, j);
					Widget wLabel = tagsAssociatedTable.getWidget(i, j + 1);
					if (wCheck instanceof CheckBox
							&& ((CheckBox) wCheck).getValue()) {
		...
		currentScenario.getAssociatedTags().setAssociatedTagNames(
				associatedTags);
		currentScenario.setLastActionTaken(SCN_LAST_ACTION_TAG);
		currentScenario.setLastActionUser(userEmail);
		// checkScenarioFields(scnReq);//not really that necessary, because it
		// would be updated when clicking the FeedBack tab
	...
	// ---------
	// Related to ticket-145
	@UiField
	Button prevScenarioButton;
	@UiField
	Button nextScenarioButton;

	@UiHandler("prevScenarioButton")
	public void onClickPrevScenarioResult(ClickEvent clickEvent) {
		positionInSearchResultsList--;
		// selectFirstTab();
		// cleanStatusLabel();
		setCurrentScenario(scenarioSearchResultsList
				.get(positionInSearchResultsList));
	}

	@UiHandler("nextScenarioButton")
	public void onClickNextScenarioResult(ClickEvent clickEvent) {
		positionInSearchResultsList++;
		// selectFirstTab();
		// cleanStatusLabel();
		setCurrentScenario(scenarioSearchResultsList
